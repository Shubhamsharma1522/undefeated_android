...
class NewChatRoom extends Component {
  ...
  componentDidMount() {
    ...
    this.props.getSportList({contest_type: 'HUDDLE'});
    try {
      let params = {
  ...
  submitQuery = async () => {
    ...
    const {auth_token, role} = this.props.user;
    ...
    await this.props.addNewChatRoom(params);
    await this.props.getChatRooms(params);
  };
  addUserToGroup = user => {
  ...
  renderNewBet = () => {
    const profilePicPlaceHolder =
      APPLICATION_IMAGE_CONSTANTS.huddleWatchPartyCoverImg;
    const {
      image,
      title,
      description,
    ...
    return (
      <View style={styles.newBetContainer}>
        <View>
          <Text style={styles.createPartyTitle}>Create a New Huddle</Text>
          {/* <View>
            ...
            <TouchableOpacity onPress={this.changeImage}>
              <View style={styles.imageContainer}>
                <Image
                  ...
                  style={styles.imageHeader}
                />
                <Text style={styles.chooseImageText}>Upload Cover Photo</Text>
              </View>
            </TouchableOpacity>
          </View>
        </View>
        ...
        <View style={styles.titleContainer}>
          <TextInput
            allowFontScaling={false}
            placeholder={APPLICATION_CONSTANTS.team_one + '(Optional)'}
            placeholderTextColor={COLORS.lightGrey}
            style={styles.titlePlaceholder}
            value={team_one}
        ...
        <View style={styles.titleContainer}>
          <TextInput
            allowFontScaling={false}
            placeholder={APPLICATION_CONSTANTS.team_two + '(Optional)'}
            placeholderTextColor={COLORS.lightGrey}
            style={styles.titlePlaceholder}
            value={team_two}
        ...
        <View style={styles.privateGroupContainer}>
          <View>
            <Text allowFontScaling={false} style={styles.privateChat}>
              {APPLICATION_CONSTANTS.privateChatRoom}
            </Text>
            <Text
              allowFontScaling={false}
              style={styles.privateChatDescriptions}>
              {APPLICATION_CONSTANTS.privateChatRoomDescription}
            </Text>
          </View>
          <Switch
...
mapDispatchToProps = dispatch => {
  return {
    ...
    addNewChatRoom: params => dispatch(TASKS.addNewChatRoom(params)),
    getChatRooms: params => dispatch(TASKS.getChatRooms(params)),
    getSportList: params => dispatch(TASKS.getSportList(params)),
  };
};
export default connect(mapStateToProps, mapDispatchToProps)(NewChatRoom);
