...
class NewContestWatchParty extends Component {
  ...
  componentDidMount() {
    this.props.getSportList({contest_type: 'PICKEM'});
  ...
  submitQuery = async () => {
    ...
    await this.props.addNewPickemWatchParty(params);
    await this.props.getPickemWatchParty(params);
  };
  addUserToGroup = user => {
  ...
  renderNewBet = () => {
    const profilePicPlaceHolder =
      APPLICATION_IMAGE_CONSTANTS.pickemWatchPartyCoverImg;
    const {
      image,
      title,
      description,
    ...
    return (
      <View style={styles.newBetContainer}>
        <View>
          ...
          <Text style={styles.createPartyTitle}>CREATE A PICK'EM CONTEST</Text>
        </View>
        <View />
...
mapDispatchToProps = dispatch => {
  return {
    ...
    addNewPickemWatchParty: params =>
      dispatch(TASKS.addNewPickemWatchParty(params)),
...
export default connect(
  mapStateToProps,
  mapDispatchToProps,
)(React.memo(NewContestWatchParty));
